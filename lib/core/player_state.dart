...
class PlayerController extends ChangeNotifier {
  ...
  Future<void> ciclarModoAleatorio({LikedStore? likes, HomeStore? home}) async {
    // En la vía libre el aleatorio se queda quieto: los dos modos se apoyan en
    // `api.setShuffle` y en mandar offsets a Spotify Connect, y allí no hay ni
    // una cosa ni la otra. Antes que fingir un botón que no hace nada, no hace
    // nada y se sabe.
    if (libre != null) return;
  ...
  Future<void> activarAleatorioInteligente({LikedStore? likes, HomeStore? home}) async {
    // Igual que [ciclarModoAleatorio]: sin Connect no hay a dónde mandar la
    // cola que arma este método.
    if (libre != null) return;
    try {
  ...
  Future<void> playLista(
    ...
    // La vía libre lleva su propia cola en memoria: ni hay dispositivo Connect
    // al que mandar la lista ni sirve la posición inicial, que allí la fija el
    // reproductor local al abrir la pista.
